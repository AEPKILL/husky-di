...
export function createLabHost({
	...
	let nextUntrackedId = 0;
	const custom = createLabCustomServices({
		instanceId,
		...
		targets: {
			exposure(target) {
				if (
					...
					acceptor.state.status === RpcStateStatusEnum.closed
				)
					return undefined;
				if (target.scope === LabCustomScopeEnum.nodeGlobal) return acceptor;
				if (target.scope !== LabCustomScopeEnum.nodePeer) return undefined;
				return [...peers.values()].find(
					(entry) =>
						entry.id === target.peerId &&
	...
	const shipping = {
		quote: (from: string, to: string, kg: number) =>
			record(
				"acceptor",
	...
	function record<T>(
		...
		const traceId = traceContext.get() ?? `untracked-${++nextUntrackedId}`;
		...
		return recorder.run(
			...
			() =>
				traceContext.run(traceId, () =>
					entry.peer.resolve(REMOTE_LAB_BROWSER_SERVICE).receive(message),
				),
		);
	}
	...
	return {
		...
		openPeer(peer) {
			...
			const run = <T>(
				...
				args: readonly unknown[],
				operation: () => T | Promise<T>,
			) => record(entry.id, LAB_SERVICE_NAMES.lab, method, args, operation);
			const service: ILabService = {
				identify: () => entry.id,
				identifyServer: () => instanceId,
				quote: (from, to, kg) =>
					run("quote", [from, to, kg], () => quote(from, to, kg)),
				echo: (value) => run("echo", [value], () => value),
				fail: () =>
					run("fail", [], () => {
						throw new Error("Intentional Lab handler failure.");
					}),
				report: (pause, delayMs, signal) =>
					run("report", [pause, delayMs], async () => {
						const traceId =
							traceContext.get() ?? `untracked-${++nextUntrackedId}`;
