/**
 * @overview Evidence for bounded example recordings and detached message capture at Transport boundaries.
 * @author AEPKILL
...
it("EXAMPLE-LAB-RECORD-001 observes local admission once without exposing unrecognized Transport bytes", async () => {
	const recorder = createLabRecorder(LabSideEnum.browser);
	...
	assert.deepEqual(
		recorder.snapshot().entries.map((entry) => entry.transportMessage),
		[LabTransportDirectionEnum.sent, LabTransportDirectionEnum.received].map(
			(direction) => ({
				type: "unknown",
...
it("EXAMPLE-LAB-RECORD-001 retains all handshake fields and original JSON in both directions once per message", async () => {
	...
	for (const frame of frames) {
		const payload = JSON.stringify(
			{
				...frame,
				extension: {
					text: "complete-field".repeat(2_000),
					flags: [true, null],
				},
			},
			null,
		...
		assert.deepEqual(sent.transportMessage, sent.handshakeFrame);
		assert.deepEqual(incoming.transportMessage, incoming.handshakeFrame);
...
it("EXAMPLE-LAB-RECORD-001 keeps malformed and oversized messages byte-only and bounds full handshake history", async () => {
	...
	assert.ok(
		recorder
			...
			.entries.every(
				(entry) =>
					entry.transportMessage?.type === "unknown" &&
					entry.transportMessage.payload === undefined,
			),
	);
...
it("EXAMPLE-LAB-MESSAGES-001 records actual protocol messages once in both directions and detaches payloads before settlement", async () => {
	const recorder = createLabRecorder(LabSideEnum.browser);
	const connections = new Subject<IRpcConnection>();
	const messages = new Subject<Uint8Array>();
	let settle = () => {};
	...
	const connection: IRpcConnection = {
		...
		async send() {
			sendCount += 1;
			if (sendCount === 1) await admission;
			else if (sendCount === 2) throw failure;
		},
		async close() {},
	};
	const adapter = createObservedConnectorAdapter(
	...
	for (const kind of kinds) {
		const payload = JSON.stringify(
			{
				kind,
				message: { kind: "call", service: "example", method: "echo" },
				extension: { text: "complete-message".repeat(2_000) },
			},
			null,
			2,
		);
		...
		messages.next(incoming);
		incoming.fill(0);
		const [receivedEntry, sentEntry] = recorder.snapshot().entries;
		for (const [entry, direction] of [
			[receivedEntry, LabTransportDirectionEnum.received],
			[sentEntry, LabTransportDirectionEnum.sent],
		] as const) {
			assert.equal(entry.handshakeFrame, undefined);
			assert.deepEqual(entry.transportMessage, {
				type: kind,
				connectionId: "Browser-connection-1",
				direction,
				bytes: length,
				outcome:
					direction === LabTransportDirectionEnum.sent && sendCount === 2
						? "failed"
						: "fulfilled",
				payload,
			});
			Object.assign(entry.transportMessage ?? {}, { payload: "changed" });
		}
		assert.equal(
			recorder.snapshot().entries[0].transportMessage?.payload,
			payload,
		);
		assert.equal(
			recorder.snapshot().entries[1].transportMessage?.payload,
			payload,
		);
	}
	assert.equal(received, kinds.length * 2);
	assert.equal(recorder.snapshot().entries.length, kinds.length * 2);
	assert.equal(
		JSON.stringify(recorder.snapshot()).includes(failure.message),
		false,
	);
...
it("EXAMPLE-LAB-NETWORK-001 associates fresh frames without changing payloads or prior snapshots and retains browser identity across clear", () => {
	...
	for (const recorder of [browser, node]) {
		...
		assert.equal(
			after.entries[2].transportMessage?.sessionId,
			"current-session",
		);
		...
		assert.equal(before.entries[1].transportMessage?.sessionId, undefined);
		assert.equal(fresh.sessionId, undefined);
...
it("EXAMPLE-LAB-NETWORK-001 retains connection association through delayed sends, recovery history eviction, and clear", async () => {
	...
	assert.equal(
		recorder
			.snapshot()
			.connections.find((entry) => entry.connectionId === replacementConnection)
			?.sessionId,
		"current-session",
	);
	await observed[1].close();
	assert.ok(
		!recorder
			.snapshot()
			.connections.some(
				(entry) => entry.connectionId === replacementConnection,
			),
	);
	assert.equal(
		recorder.snapshot().entries[0].handshakeFrame?.sessionId,
	...
	const ack = encode({
		kind: "ack",
		sessionId: "extension-only",
		receivedThrough: 2,
	});
	await observed[1].send(ack);
	assert.equal(
		recorder.snapshot().entries[0].transportMessage?.sessionId,
		"current-session",
	);
	assert.equal(
		recorder.snapshot().entries[0].transportMessage?.connectionId,
		replacementConnection,
	);
	assert.equal(
		recorder.snapshot().entries[0].transportMessage?.payload,
		new TextDecoder().decode(ack),
	);
	assert.equal(recorder.snapshot().entries[0].handshakeFrame, undefined);
	connections.next({ ...connection, async send() {} });
