...
describe("Remote Lab scenarios", () => {
	...
	it("EXAMPLE-LAB-RPC-001 returns a real shipping quote and records sample payloads separately from safe diagnostics", async () => {
		...
		try {
			...
			assert.deepEqual(
				{
					...(await client.traceContext.run("shipping-trace", () =>
						shipping.quote("Shanghai", "Beijing", 2),
					)),
				},
				{
					from: "Shanghai",
			...
			assert.ok(
				recorded.recording?.entries?.some(
					(entry: { transportMessage?: { payload?: string } }) =>
						entry.transportMessage?.payload?.includes('"metadata"') &&
						entry.transportMessage.payload.includes("shipping-trace"),
				),
				"The correlation trace must travel as call metadata.",
			);
	...
	it("EXAMPLE-LAB-VALUE-001 distinguishes legal values, local preflight rejection, handler failure and unknown methods", async () => {
		...
		try {
			...
			assert.deepEqual(
				structuredClone(
					await client.traceContext.run("legal-value", () =>
						lab.echo({
			...
			for (const value of [
				...
				await assert.rejects(
					client.traceContext.run("invalid-value", () => lab.echo(value)),
					TypeError,
				);
			...
			await assert.rejects(
				client.traceContext.run("handler-failure", () => lab.fail()),
			...
			assert.equal(
				await client.traceContext.run("healthy-after-errors", () =>
					lab.echo("still connected"),
				),
				"still connected",
			);
		} finally {
	...
	it("EXAMPLE-LAB-DEBUG-001 keeps a canceled caller terminal after the paused handler resumes", async () => {
		...
		try {
			...
			const report = client.traceContext.run("paused-report", () =>
				lab.report(true, 0, controller.signal),
			);
	...
	it("EXAMPLE-LAB-RECOVERY-001 resumes a paused report through the retained facade without entering its handler twice", {
		...
		try {
			...
			const report = client.traceContext.run("recovery-report", () =>
	...
	it("EXAMPLE-LAB-EXPOSURE-001 / PEERS-001 isolates per-peer cleanup, global cleanup and application fanout", async () => {
		...
		try {
			...
			for (const service of global)
				await assert.rejects(
					clients[0].traceContext.run("revoked", () =>
						service.quote("A", "B", 1),
					),
					hasCode(RpcExceptionCodeEnum.unknownService),
				);
			...
			assert.equal(
				(
					await clients[1].traceContext.run("restored", () =>
						global[1].quote("A", "B", 1),
					)
				).amount,
				16,
			);
