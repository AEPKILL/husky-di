...
none of the built-in `husky-di-rpc/1` or `husky-di-rpc/2` internals are public extension points. The
...
Call requests carry separate `args` and optional `metadata` snapshots. Preserve
metadata across delivery and replay, and include its weight in retained request
budgets. Incoming metadata must be a record normalized through the host's
`normalizeApplicationValue()`; forged snapshots and non-record roots are Protocol
faults. An absent field means no metadata. The built-in profile omits empty
outgoing metadata and accepts the optional `Call.metadata` record; older endpoints
may ignore it. See `RPC-INTERCEPT-005` in [SPECIFICATION.md](SPECIFICATION.md).
...
A stream-capable Session supplies `prepareStream(request, observer)`, returning
the same `start()`/`cancel()` control shape used for unary preparation. The
request uses canonical `service`/`method`, detached `args`, and optional metadata.
The built-in v2 wire record maps `request.method` to `stream-open.member`.
An absent stream port or a v1 Session means stream capability is unavailable;
fail locally before sending an unknown record.

`IRpcProtocolStreamObserver` receives `next(snapshot)`, `complete()`, or
`error(RpcCallFailure)`. Incoming `openIncomingStream(request, observer)` returns
an `IRpcProtocolIncomingStream` with `finish(code?)`, or `undefined` on admission
pressure. Publish one durable disposition before notifying terminal observers,
and stop the source when its incoming handle is finished. Only host-normalized
Application Value snapshots may cross the observer port.

Keep stream identities separate from unary call identities while sharing the
direction sequence, receipt ACKs, immutable replay queue, retained-byte budgets,
and fair scheduler. Retain source execution during Recovery and replay original
records before new sequence allocation. A stream must never resubscribe under a
new Session Incarnation. Resource exhaustion consumes its protected safe
`unavailable` terminal allowance; malformed grammar or continuity faults terminate
the Session. See `RPC-STREAM-001` through `RPC-STREAM-010` for exact lifecycle rules.
