...
**Status:** Normative for the implemented Remote contracts and runtime
**Profiles:** `husky-di-rpc/1` (unary), `husky-di-rpc/2` (unary and Observable streams)

## 1. Scope

This specification defines the current contract of `@husky-di/remote`: a bidirectional unary and RxJS Observable RPC Framework,
...
The v2 profile supports bidirectional unary calls and Observable streams; the v1 profile retains unary compatibility. Notifications, automatic Container integration, business
authentication/authorization/rate limiting, rewriting service routes or business arguments, cross-process persistent Session
...
- **Protocol**: the replaceable semantic engine supplied through role-specific Connector and Acceptor factories.
  The built-in Protocol implements `husky-di-rpc/1` for unary calls and `husky-di-rpc/2` for unary calls and streams.
...
- **Call Metadata**: an application-owned record conveyed from caller to callee separately from business arguments.
- **Interceptor**: an Owner-configured application function that wraps outgoing unary calls and stream subscriptions,
  and incoming handlers, through a single-use Observable continuation and may attach or inspect Call Metadata.
- **Remote Stream Subscription**: one caller subscription with its own execution, observer, cancellation, and terminal.
- **Remote Stream Identity**: `(Session Incarnation, originating direction, streamId)`, independent of Call Identity.
- **Static Observable Member**: an Observable data property captured once at exposure; within one Peer,
  subscriptions to the same captured instance share one lazy source connection.
- **Stream Terminal**: the first durable completion, error, or cancellation disposition; unsubscribe is silent locally.
...
provable. Stream source dispatch **MUST** obey the same retained-incarnation constraint. Loss of that evidence **MUST NOT** be described as exactly-once or transparently retried under a new
Call Identity.

**RPC-BASE-002 — Observable ownership.** Subscribing or unsubscribing from public Owner/Peer `state$`,
`event$`, and `peers$` Observables **MUST NOT** create, start, stop, or own a Transport, Session, listener, or
handler. Those resources **MUST** be owned by role-specific lifecycle methods. A remote member subscription
**MUST** own only its remote stream execution and cancellation lifecycle, never the containing Owner or Transport.
...
The package currently remains at `0.0.0`. The specification and built-in Protocol profile versions describe
the behavioral contract independently of a future package release.
...
shape. The preserved `RpcConnectorConnectOptions` declaration is the sole compatibility exception: it
**MUST** retain its readonly `adapter` and optional readonly `signal` shape colocated with `IRpcConnector`,
with matching package-private schema validation at runtime. A type-only facade, readonly wrapper, or
semantic-brand wrapper **MAY** layer on a derived type. Focused
...
`createRemoteServiceDescriptor`, `createRpcConnector`, `createRpcAcceptor`, `createRpcReconnectionConnector`,
...
`RpcCallContext`, `RpcInterceptor`,
`RpcConnectorConnectOptions`, `RpcConnectorRuntimePolicyOptions`, `RpcAcceptorRuntimePolicyOptions`,
`IRpcConnectorReconnection`, `CreateRpcReconnectionConnectorOptions`, `RpcConnectorAdapterFactory`,
...
`RpcExceptionCodeEnum`, `RpcIncomingCallKindEnum`, `RpcProtocolSessionTransitionTypeEnum`, and
`RpcWireRecordKindEnum` plus the complete
...
`RpcProtocolIncomingCallReservation`), stream observer/control types
(`IRpcProtocolStreamObserver` and `IRpcProtocolIncomingStream`),
...
**RPC-VALUE-001 — Data domain.** Caller arguments, Call Metadata, successful results, and safe error details **MUST** normalize
...
limits below; bytes mean UTF-8 bytes and root depth is one. The complete wire-tree allowance is fixed and
**MUST NOT** expand when arguments and Call Metadata are both present; each Application Value remains subject
to its own allowance:

| Dimension | Hard limit |
| --- | ---: |
| Complete Transport message | `1,048,576 B` |
| One arguments/metadata/result/error-details compact-JSON budget weight | `1,000,000 B` |
...
A Descriptor is opaque and selects a non-empty `members` map. Each selected member has an explicit `kind`: `function`
for unary calls, `observable-function` for methods returning an RxJS `Observable<T>`, or `observable` for a static
Observable data property. `methods` is not accepted. The reserved key `then` is rejected.

**RPC-DESC-001 — Descriptor shape.** `createRemoteServiceDescriptor()` **MUST** retain a detached, frozen snapshot of
`wireName` and the non-empty `members` map without exposing either on the Descriptor object. `methods` alone
or together with `members` **MUST** be rejected; it is not a compatibility alias.

**RPC-DESC-002 — Member validation.** A `function` **MUST** be a callable data property; an `observable-function`
**MUST** return an RxJS Observable directly and **MUST NOT** accept an AbortSignal slot; an `observable` **MUST** be a
non-function RxJS Observable data property. Accessors, missing members, AsyncIterable values, Promise-wrapped Observables,
and unknown member kinds **MUST** be rejected atomically.

**RPC-DESC-003 — Reserved names.** `then` **MUST NOT** be selected as a member.

**RPC-DESC-004 — Exposure snapshot.** Exposure **MUST** walk the implementation prototype chain without invoking getters,
then capture static Observable identity once. Own and inherited data properties **MUST** be supported; accessor
or reflective-read failures **MUST** fail the whole exposure. Static validity **MUST** use RxJS `isObservable()`.

**RPC-DESC-005 — Facade.** A resolved facade **MUST** be frozen, null-prototype, non-thenable, and expose only selected
members. Static facade properties **MUST** be readonly and return the same Observable object on repeated reads,
even when the source interface property is mutable. Each subscription **MUST** be independent; within one Peer,
static sources **MUST** connect lazily and tear down after the last subscription, including sharing across routes
that capture the same Observable instance. Different Peers **MUST NOT** share that source subscription.
...
`RpcException` with code `protocol` and may preserve only the trusted local cause. A supplied `interceptor`
**MUST** be callable or `undefined`; an invalid value **MUST** synchronously throw `TypeError` before Protocol
construction.
...
null-prototype facade containing only selected unary/Observable method closures and readonly static Observable
properties. A closure **MUST NOT** depend on facade `this`. Unary closures **MUST** return Promises; stream methods
**MUST** return cold RxJS Observables whose subscription starts remote work.
...
usable across Recovery and **MUST** still be obtainable after close; unary invocation then rejects `unavailable`
and stream subscription errors with `unavailable`.
...
typing. Both built-in profiles **MUST NOT** reflectively validate business arity.
...
availability, Application Value arguments snapshot, optional outgoing interception, Call Metadata snapshot,
then capacity. Calling an interceptor's `next()` **MUST** recheck cancellation and owner/peer availability
before preparing an invocation, as specified by `RPC-INTERCEPT-002`. Failure before capacity **MUST** create no Pending
...
**MUST NOT** change the public Promise, stream observer terminal, or matching finished observation. A local
stream unsubscribe **MUST** end silently while its first durable terminal still governs replay and cleanup.

**RPC-CALL-007 — Error guarantees.** For unary calls and stream opening rejection, `unavailable` **MUST** mean
Definite Non-Execution. A stream already executing **MAY** terminate with `unavailable` on bounded resource
exhaustion; earlier emissions or application effects are not rolled back. `outcome-unknown` **MUST** be used only
for admitted outgoing work that lost authoritative terminal evidence.
...
calling the incoming interceptor or captured handler. Cancellation/force that wins while queued **MUST** prevent dispatch; a running
...
#### 6.3.1 Observable interception and metadata

```typescript
type RpcCallContextBase = {
  readonly peer: IRpcPeer;
  readonly service: string;
  readonly method: string;
};

export type RpcCallContext = RpcCallContextBase & (
  | {
...
**RPC-INTERCEPT-001 — Owner registration.** Each Owner **MAY** receive one `interceptor` at construction.
`callInterceptor` and the exported type name `RpcCallInterceptor` **MUST NOT** remain aliases. An options object
containing the old key, including alongside `interceptor`, **MUST** be rejected. Framework **MUST** retain that function independently of later option mutation and apply it to outgoing
invocations and eligible incoming handler jobs on every Peer owned by that Owner, including future Acceptor
Peers and Peers retained through Recovery. Context **MUST** identify the stable Peer, locally canonical service
and method, direction, and applicable cancellation signal. Each stream subscription **MUST** have an AbortSignal tied to its teardown;
unary signal behavior **MUST** retain the selected cancelable contract. Omitting the option **MUST** preserve ordinary
invocation behavior. Construction **MUST NOT** execute the interceptor.

**RPC-INTERCEPT-002 — Outgoing preparation.** Framework **MUST** validate the control slot, initial cancellation,
availability, and detached business-argument snapshot before entering the outgoing interceptor. Each invocation's
metadata **MUST** initially be an empty record; the interceptor **MAY** replace `context.metadata` with an
Application Value record. `next()` **MUST** check the original signal's aborted state and current Owner/Peer
availability, snapshot and validate the metadata, and only then reserve Pending capacity. Metadata **MUST** have
a plain record root, obey `RPC-VALUE-001` through `RPC-VALUE-005`, and be detached from application mutations
before it reaches the Protocol. Invalid metadata **MUST** reject `TypeError`; cancellation and unavailable state
**MUST** retain their ordinary safe errors and create no Pending Invocation. Asynchronous work before `next()` **MUST** be expressed through Observable composition and
remain application preparation without a Pending entry, wire identity, or observation. Unsubscribe before
continuation **MUST** tear down preparation and prevent any `stream-open`. A delayed continuation **MUST** recheck
cancellation and availability.

**RPC-INTERCEPT-003 — Continuation and result.** `next()` and the interceptor **MUST** return RxJS Observables.
`next()` **MUST** be single-use; another call or a call after teardown **MUST** fail with `TypeError` without
starting further work. The Framework **MUST** wrap unary handler values/Promises at its Observable boundary.
A unary result with one emission **MUST** resolve that value, no emissions **MUST** resolve `undefined`, and a
second emission **MUST** fail the current call with safe `handler-failed` and cancel its execution. Streams
**MUST** retain multiple emissions. An interceptor **MAY** short-circuit with its own Observable or use RxJS
operators around `next()` to transform values and errors. Outgoing local throws/operator errors **MUST** remain
local errors. Short-circuiting **MUST NOT** create Protocol work or observations. Framework **MUST NOT** retry
interceptors, invoke them per item, or rerun them during Recovery replay.

**RPC-INTERCEPT-004 — Incoming execution.** Framework **MUST** enter the incoming interceptor only for a captured
known handler after its Session and Owner permits are acquired. Unknown routes and jobs canceled while queued
**MUST NOT** run it. Context **MUST** contain the detached, deeply frozen received metadata record, or an empty
record when metadata is absent. Incoming `next()` **MUST** check the call's terminal state before invoking the
captured handler, preserve its implementation receiver, business arguments, and optional cancellation slot,
and prevent delayed dispatch after cancellation or termination. The interceptor's result **MUST** pass through
ordinary Application Value normalization; its throw, invalid Observable, or operator/source error **MUST** become `handler-failed` without exposing
its details or closing a healthy Peer. Framework **MUST** retain handler permits until both the interceptor and
any unary handler started through `next()` have actually settled, even when the interceptor returns or throws before
its handler settles. A cancellation signal **MUST NOT** count as settlement, and late results **MUST NOT** replace
a selected terminal.

**RPC-INTERCEPT-005 — Protocol custody and compatibility.** Call Metadata **MUST** travel separately from
business arguments through optional Protocol request metadata and built-in `Call.metadata`/`StreamOpen.metadata`. Empty
outgoing metadata **MUST** omit both fields. A non-`undefined` SPI field **MUST** be a genuine Framework Application
snapshot with a record root; a present wire field **MUST** be an Application Value record. Metadata weight
**MUST** participate in Pending, incoming-work, and retained replay accounting and **MUST** share the request
payload's release lifecycle. Replay **MUST** preserve the original metadata snapshot. The optional field retains
unary profile `husky-di-rpc/1` and also applies to v2; a combined arguments/metadata record that exceeds its complete wire limits **MUST**
reject `unavailable` before Outgoing Admission without sending it. An older endpoint **MAY** ignore metadata
under `RPC-WIRE-005`. Applications **MUST NOT** infer
metadata support or security enforcement from that profile identifier or successful call completion.

**RPC-INTERCEPT-006 — Isolation and observations.** Each call or subscription interception **MUST** have its own context and metadata
association; concurrent calls or subscriptions **MUST NOT** share mutable Framework context. Framework **MUST NOT** generate a
trace identifier, create ambient context, or automatically forward incoming metadata to a later outgoing call.
Applications **MAY** establish such context around `next()`. Call Metadata **MUST NOT** enter public events or
Framework errors. Events for a started outgoing RPC **MUST** describe its actual Protocol outcome, independently
of a result or error subsequently replaced by the outgoing interceptor.
...
type RpcStreamObservationBase = RpcCallObservationBase & {
  readonly direction: RpcCallDirectionEnum;
  readonly service?: string;
  readonly method?: string;
};
type RpcStreamOpenedEvent = RpcStreamObservationBase & {
  readonly type: RpcEventTypeEnum.streamOpened;
};
type RpcStreamFinishedEvent = RpcStreamObservationBase & {
...
initial abort, and Remote Resource Rejection **MUST NOT** create a pair. Stream observations **MUST** use
`stream-opened`/`stream-finished` with the same local pairing and reentrant-publication guarantees.
...
code. For unary calls, `terminated` **MUST** be incoming-only and carry no code, Error, or Session reason. Stream
completion **MUST** report `fulfilled`; stream error **MUST** report `rejected` with its safe code; unsubscribe
**MUST** report a terminal observation without notifying the canceled RxJS observer.

**RPC-EVENT-004 — Payload safety.** Events **MUST NOT** contain raw wire, args, Call Metadata, result, details, thrown value,
...
history, redaction callback, exporter, automatic trace propagation, or console sink. Applications **MAY** record
payloads or explicitly propagate metadata at caller/handler boundaries and Interceptors they own.
...
### 6.6 Observable stream subscriptions

**RPC-STREAM-001 — Observable member contract.** A Descriptor **MUST** identify each remote route as
`function`, `observable-function`, or `observable`, with the validation, snapshot, `then`, and readonly rules
in Section 5. Only direct RxJS Observables **MUST** be supported; AsyncIterable, Promise-wrapped Observable,
non-Observable result, accessor, or a stream method with an AbortSignal parameter **MUST NOT** be accepted.

**RPC-STREAM-002 — Subscription isolation.** Each `subscribe()` **MUST** create a distinct Remote Stream Identity
and terminal/cancellation lifecycle. A method stream **MUST** call its handler and subscribe to its result anew
for each remote subscription. Static sources **MUST** remain lazy after exposure and connect once for the first
subscriber to that captured instance on one Peer; subsequent subscribers on that Peer **MUST** share the source
without replaying earlier items. The last subscription ending by complete, error, or cancel **MUST** tear down
the source. Each remote subscription **MUST** still enter interception independently, including static members.

**RPC-STREAM-003 — Stream wire grammar.** Only v2 **MUST** carry `stream-open`, `stream-next`, `stream-complete`,
`stream-error`, and `stream-cancel`. They **MUST** use the same direction-level sequence, ACK, replay barrier,
validation, and immutable message rules as unary records. Each stream **MUST** use a canonical independent
stream ordinal scoped to Session Incarnation and direction. A replayed open **MUST NOT** dispatch or subscribe
again. The first durable complete/error/cancel **MUST** win; later valid records **MUST** be consumed and ACKed
without notifying observers or reopening execution. The receiver **MUST** retain bounded normalized duplicate
evidence charged to Session and Owner budgets; once that evidence is compacted, old sequence values **MUST**
remain non-dispatching and ACKable using the receive high-watermark. Malformed identity, sequence gaps, changed replay bodies while their bounded duplicate evidence is retained,
ACK overclaims, or unsafe decoding **MUST** fault the Session, never synthesize a business stream error.

**RPC-STREAM-004 — Cancellation.** Local unsubscribe **MUST** immediately and silently end its observer and
request cooperative `stream-cancel` for admitted work. It **MUST NOT** synthesize `complete` or `error`, cancel
other subscriptions, or close the Peer. Before admission it **MUST** retract pending work without sending open
or cancel. Cancellation **MUST** tear down the corresponding source subscription and release retained execution
resources; late source notifications **MUST NOT** become observer notifications.

**RPC-STREAM-005 — Safe terminal mapping.** Each normalized source value **MUST** become one ordered
`stream-next` and observer `next`. Source completion **MUST** become `stream-complete` and observer `complete`.
Source error, handler synchronous throw, non-Observable handler result, source subscription failure, incoming
interceptor/operator error, or item normalization failure **MUST** terminate only that stream with safe
`stream-error(handler-failed)` and observer `error(RpcException)`. Raw errors and source values **MUST NOT**
leak through error details.

**RPC-STREAM-006 — Recovery.** Recovery **MUST** preserve open streams, source execution, subscriber association,
and unacknowledged immutable `(seq, SemanticMessage)` entries. Replacement bindings **MUST** replay retained
records in order before newly sequenced records, without new open or source subscription. Sources **MAY**
continue emitting during Recovery within existing finite budgets. A new Session Incarnation **MUST NOT**
automatically resubscribe old streams. Recovery expiry, Session reclamation, remote graceful close, or local
force **MUST** error pre-admission streams with `unavailable` and admitted streams lacking authoritative
terminal evidence with `outcome-unknown`; none **MUST** become observer `complete`. `remote-terminated` **MUST**
remain a lifecycle reason, and `outcome-unknown` **MUST NOT** appear in a wire error.

**RPC-STREAM-007 — Resource limits.** Every open, item, safe error, terminal, and cancel **MUST** participate in
existing retained-byte, replay, queue, and fairness limits. Unary and stream work **MUST** share their aggregate
finite capacity. Resource exhaustion **MUST** stop that stream, release source execution, and use its protected
terminal slot to send safe `unavailable`. Stream cancellation **MUST** retain its protected control allowance.
No independent backpressure protocol or unbounded item queue **MUST** be introduced. Terminal/cancel ACK
**MUST** release unique payload charges while high-watermarks and bounded dedupe evidence survive the Session.

**RPC-STREAM-008 — Interceptor continuation.** Each stream opening **MUST** enter the Owner's Observable
`interceptor` once on each participating side, following Section 6.3.1. Item transformation **MUST** use the
returned Observable pipeline; terminal replay **MUST NOT** re-enter interception. Static sharing **MUST NOT**
share mutable interception context or duplicate the underlying source connection. Old option and type aliases
**MUST** be rejected.

**RPC-STREAM-009 — Event payloads.** Framework **MUST** publish only `stream-opened` and `stream-finished`
observations, never a per-item framework event. Every published opening **MUST** have exactly one matching
terminal observation under reentrancy and cancellation. Public `event$` and the internal observation sink
**MUST** expose only opaque local observation identity, stable Peer, known direction/route, duration, outcome,
and safe terminal code. Items, metadata, source, arguments/results, raw Error, and wire stream identity
**MUST NOT** be included.

**RPC-STREAM-010 — Profile compatibility.** Built-in fresh negotiation **MUST** prefer `husky-di-rpc/2` and
offer `husky-di-rpc/1` for unary-only peers. v1 **MUST** preserve existing unary grammar and semantics. A stream
subscription on a v1 Session **MUST** error with `unavailable` before any stream record is sent and **MUST NOT**
fault the Session. v2 peers **MUST** support both unary and stream members. An unknown or profile-illegal
semantic kind received on an established Session **MUST** remain a Protocol fault.
...
export interface IRpcProtocolCallRequest {
  ...
  readonly metadata?: IRpcApplicationSnapshot<IRpcApplicationRecord>;
}

export interface IRpcProtocolStreamObserver {
  next(value: IRpcApplicationSnapshot): void;
  complete(): void;
  error(code: RpcCallFailure): void;
}

export interface IRpcProtocolIncomingStream {
  finish(code?: RpcCallFailure): void;
}
...
For streams, the semantic ports are `prepareStream(request, observer)` and
`openIncomingStream(request, observer)`. The optional ports preserve unary-only custom Protocols: absence
means stream capability is unavailable. Outgoing preparation returns the existing `start`/`cancel` control;
incoming `finish(code?)` commits completion when code is absent or the selected safe error/cancel otherwise.
Observer values are host-normalized snapshots; Section 6.6 defines their ordering and terminal rules.
...
advancement, no args or Call Metadata retention, and no incoming event. `true` **MUST** call `consume` exactly once and
...
**RPC-WIRE-001 — Atomic profile.** The built-in profile identifiers **MUST** be exactly `husky-di-rpc/1`
for unary and `husky-di-rpc/2` for unary plus streams. Each profile **MUST** atomically fix Codec, grammar, bearer-credential security, Recovery, deduplication,
...
`args`, `metadata`, `value`, or `details` **MUST** treat every member as application data. Duplicate names always invalidate
the record.
...
The v1 sequenced semantic union, unchanged in v2, is:
...
The v2 sequenced semantic union additionally contains:

```text
StreamOpen = {
  kind: "stream-open", streamId: CanonicalStreamOrdinal,
  service: NonEmptyIdentifier, member: NonEmptyIdentifier,
  args: ApplicationValue[], metadata?: ApplicationRecord
}
StreamNext = { kind: "stream-next", streamId: CanonicalStreamOrdinal, value: ApplicationValue }
StreamComplete = { kind: "stream-complete", streamId: CanonicalStreamOrdinal }
StreamError = { kind: "stream-error", streamId: CanonicalStreamOrdinal, error: Error.error }
StreamCancel = { kind: "stream-cancel", streamId: CanonicalStreamOrdinal }
SemanticMessageV2 = Call | Cancel | Result | Error | StreamOpen | StreamNext |
                    StreamComplete | StreamError | StreamCancel
```

`CanonicalStreamOrdinal` has the same canonical integer spelling as `CanonicalCallOrdinal`,
but uses an independent direction-local sequence and identity domain. Static stream opens carry empty `args`.
...
and compare exactly. Wire method `then` **MUST** be a profile violation. `args` **MUST** be an array. Optional
`metadata` **MUST** have a record root; `null`, arrays, and primitive roots **MUST** be profile violations. Missing
...
unacknowledged, unreplayed, and contain no known seq/ACK/token/reason/identity field or Call Metadata. A receiver **MUST**
...
one, independent of Call and Stream Ordinals. Unary and all five stream kinds **MUST** share this sequence. Sequence **MUST NOT** wrap or reset during a Session Incarnation.
...
### 9.4 Logical Call and stream ledgers
...
the validated args, Call Metadata, handler, or event. Capacity success plus known route **MUST** record in-progress Remote
...
the Framework call handle, handler closure, request arguments, and Call Metadata; only the terminal/replay identity and bounded
...
replay entry **MUST NOT** leave request arguments or Call Metadata retained while the call awaits a terminal. Ordinary pressure
**MUST NOT** evict replay, call, terminal, or dedupe evidence required by this paragraph.

Stream ledgers apply the corresponding identity, replay, immutable-terminal, ACK, and high-watermark rules
above as specified by `RPC-STREAM-003`, `RPC-STREAM-006`, and `RPC-STREAM-007`; they never reuse unary Call Identity.
...
call and stream ledgers, live source subscriptions, binding epoch, and resume-attempt high-watermark. The token **MUST** remain stable and **MUST NOT**
rotate within that incarnation. A process restart or retained-state loss **MUST** end the incarnation; both built-in profiles
...
Call and Remote Stream Identities, replay entries, handler/source-dispatch evidence, live stream subscriptions,
and membership position. Connection replacement
alone **MUST NOT** settle a call or stream, resubscribe a source, or emit a second `peer-opened`.
...
export type CreateRpcReconnectionConnectorOptions = RpcConnectorOptions & {
...
**RPC-RECONNECT-001 — Construction and initial attempt.** `createRpcReconnectionConnector()` **MUST** create a
cold, opt-in, single-use supervisor from a conventional
`{ protocolFactory?, runtimePolicy?, interceptor?, adapterFactory, policy? }` configuration object whose
schema-visible keys are closed by strict schemas. After validating the complete configuration, it **MUST**
create one fresh cold Connector with the supplied `RpcConnectorOptions` and expose that same Connector as
`readonly connector`; it **MUST NOT** accept a supplied `connector`. The outer object and nested policies
**MUST** be parsed by package-private strict schemas using ordinary property access, without additional
prototype or property-descriptor restrictions; symbol metadata does not participate in these schemas, and
schema rejection **MUST** synchronously throw `TypeError` before Connector creation.
Its synchronous, argument-free Adapter Factory **MUST** be called only when an attempt begins and
...
**RPC-SEC-001 — Conditional security.** Secure built-in v1/v2 Recovery **MUST** run each fresh/replacement
...
| Unretired calls and streams combined | `256 / originating direction` |
| Incoming handler work-set | `256 jobs / 8 MiB arguments and metadata` |
| Running handlers | `16` |
...
`256 B` in addition to payload weight, including Call Metadata when present. Pending and incoming-work payload
charges **MUST** include the sum of arguments and metadata snapshot weights. Within one entry shared immutable
payload need not be double charged.
...
role policy, Observable method/static members, RxJS-only validation, readonly facade properties, unified
Observable interception, rejected legacy aliases, and custom Protocol/Adapter shape. Node consumer **MUST NOT** require DOM-only Adapter types; browser
...
behavior, facade assimilation, stream method/static subscriptions, cancel, complete/error, retained stream
Recovery, v1 unary fallback and stream rejection, and termination. Deno, Bun, and Workers **MAY** run non-blocking smoke
checks only.

**RPC-RELEASE-003 — Packed consumers.** Package verification **MUST** install the actual `pnpm pack` tarball into isolated Node ESM,
Node CJS, declaration, DOM-only, and browser-bundle consumers and resolve every public code subpath. Source
imports inside the workspace **MUST NOT** count as package evidence. Node ESM/CJS consumers **MUST** run
Observable members through installed public entrypoints; strict and DOM-only/browser consumers **MUST**
compile the same member/interceptor contracts. Private deep import **MUST** fail.

**RPC-RELEASE-004 — Package evidence.** The package **MUST** include this specification, requirement matrix,
caller and implementor documentation, and a CHANGELOG describing the current changes. The repository **MUST**
retain the normative suite. Build, code-standard, type, conformance, Protocol validation/security,
packed-consumer, and browser gates **MUST** pass without skips. This requirement does not select a release
version, require a Changeset, or prescribe repository workflow files.
...
| `RPC-VALUE`, `RPC-DESC`, `RPC-CALL`, `RPC-INTERCEPT`, `RPC-EVENT` | Framework + all Protocols | RT, TY, PC | `tests/specification.test.ts`, `tests/conformance/` | verified |
| `RPC-STREAM` | Framework + v2 Protocol | RT, TY, RP, BR, PK | `tests/specification/stream.test.ts`, `tests/protocol/`, `tests/resources/`, `tests/browser/`, `tests/package/` | verified |
...
Private source is grouped by domain module under `src/modules/`: Owner, Peer, Protocol, Transport, and
Reconnection. Each module has one `index.ts` entry point for other modules and tests; role directories keep
interfaces, implementations, factories, types, and utilities together within their module. Shared domain
contracts and reusable primitives live under `src/shared/`; shared code does not depend on modules. Package
entry points and the separate conformance tooling remain at the source root.

This specification does not prescribe private classes, reducers, codecs, queues, or scheduler objects.
...
## Appendix C. Non-normative decision lineage and source rationale
...
a Codec matrix. The evaluation also covered CBOR/CDDL and machine-readable public grammars; v1 instead keeps the
