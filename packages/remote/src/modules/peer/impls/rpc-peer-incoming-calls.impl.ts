...
import { processRpcHandlerCall } from "@/modules/peer/utils/rpc-handler-result.util";
import {
	...
	implements Pick<IRpcPeerCallLifecycle, "reserveIncomingCall">
{
	...
	reserveIncomingCall(
		...
		if (exposure === undefined) {
			...
			prepared =
				route === undefined || route.kind !== "function"
	...
	#prepareHandlerIncoming(
		{ args, method, metadata }: IRpcProtocolCallRequest,
		service: string,
		...
		let callMetadata: IRpcApplicationRecord | undefined = metadata?.value;
		...
		const runHandler = (): Promise<void> => {
			...
			const retainedMetadata = callMetadata;
			argumentsSnapshot = undefined;
			callMetadata = undefined;
			...
			return processRpcHandlerCall({
				route,
				argumentsSnapshot: retainedArguments,
				context: Object.freeze({
					...
					metadata: retainedMetadata ?? Object.freeze({}),
				}),
				interceptor: this.#interceptor,
				isSettled: () => settled,
				resolve: handlerOutcome.resolve,
			});
		};
