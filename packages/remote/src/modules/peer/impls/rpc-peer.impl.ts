...
export class RpcPeerImpl implements IRpcPeer {
	...
	resolve<T, Definitions extends RpcMemberDefinitions<T>>(
		...
		return createRpcFacade(
			descriptor,
			(method, cancelable, actualArguments, kind) =>
				kind === "function"
					? this.#callLifecycle.invoke(
							service,
							method,
							cancelable,
							actualArguments,
						)
					: this.#callLifecycle.subscribe(service, method, actualArguments),
		);
	}
	...
	openIncomingStream(
		...
		return this.#callLifecycle.openIncomingStream(request, observer);
	}
