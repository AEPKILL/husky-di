...
/** Establishes the non-borrowable control reserve before Session work is admitted. */
export function reserveRpcSessionProtectedBytes(
	ledger: IRpcRetainedBytesLedger,
): IRpcRetainedBytesReservation {
	const reservation = ledger.reserve(RPC_PROTECTED_SESSION_BYTES);
