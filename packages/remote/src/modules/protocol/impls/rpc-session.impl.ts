...
import { dispatchRpcSemanticMessage } from "@/modules/protocol/utils/rpc-semantic-dispatch.util";
import { configureRpcSessionEndpoint } from "@/modules/protocol/utils/rpc-session-endpoint.util";
import {
	registerRpcSessionRetainedBytes,
	reserveRpcSessionAndOwnerRetainedBytes,
	reserveRpcSessionProtectedBytes,
	unregisterRpcSessionRetainedBytes,
} from "@/modules/protocol/utils/rpc-session-retained-bytes.util";
import { completeRpcSessionTermination } from "@/modules/protocol/utils/rpc-session-termination.util";
...
export class RpcSessionImpl implements IRpcSession {
	...
	readonly _streams: IRpcSessionStreams;
	...
	public constructor(
		...
		this._host = host;
		this.profile = options.profile ?? RPC_PROFILE;
		this._retainedBytesLedger = retainedBytesLedger;
		this._protectedRetainedBytesReservation = reserveRpcSessionProtectedBytes(
		...
		this._incomingCalls = createIncomingCalls({
			getIncomingStreamCount: () => this._streams.incomingCount,
		...
		this._invocations = createInvocations({
			getPendingStreamBytes: () => this._streams.pendingBytes,
			codec,
			policy: host.policy,
			reserveRetainedBytes: (bytes) => this.reserveRetainedBytes(bytes),
		...
		this._delivery = createDelivery({
			...
			reserveDuplicateBytes: (bytes) => this.reserveRetainedBytes(bytes),
			counterExhausted,
			isClosed: () => this._closed,
			getBinding: () => this._binding,
			onMessage: (message) =>
				dispatchRpcSemanticMessage(
					message,
					this.profile,
					this._incomingCalls,
					this._invocations,
		...
		this._streams = createStreams({
			getOutgoingCallCount: () => this._invocations.count,
			getPendingCallBytes: () => this._invocations.pendingBytes,
			host,
			reserveRetainedBytes: (bytes) => this.reserveRetainedBytes(bytes),
			retention: this._callRetention,
			delivery: this._delivery,
			getHost: () => this._continuity.host,
			isDraining: () => this._shutdown.draining,
			onRetired: () => this._shutdown.check(),
			onFault: (error) => this._fault(RpcCloseReasonEnum.resourceFault, error),
			onCounterExhausted: () => this._shutdown.beginCounterDrain(),
		...
		this._shutdown = createShutdown({
			hasActiveStreams: () => this._streams.hasActive,
	...
	prepareInvocation(
		...
		if (
			this._invocations.count + this._streams.outgoingCount >=
			Math.min(
	...
	prepareStream(
		...
		if (this._closed || this._shutdown.draining || this.profile !== RPC_PROFILE)
			return undefined;
		return this._streams.prepare(request, observer);
	}
	...
	_completeTerminal(
		...
		try {
			...
			completeRpcSessionTermination(
				binding?.endpoint,
				this._continuity.host,
				reason,
				cause,
				this._onTerminal,
...
type RpcSessionImplDependencies = Readonly<{
	...
	readonly createStreams: RpcSessionStreamsFactory;
