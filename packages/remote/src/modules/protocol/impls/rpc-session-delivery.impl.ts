...
import {
	getRpcStreamLane,
	takeRpcStreamControl,
} from "@/modules/protocol/utils/rpc-stream-lane.util";
...
export class RpcSessionDeliveryImpl implements IRpcSessionDelivery {
	...
	readonly _duplicates: IRpcSessionDuplicateEvidence;
	...
	readonly _pendingStreams: (() => IRpcReplayReservation | undefined)[] = [];
	_preferStream = true;
	...
	_preferControl = true;
	_lastControlLane: string | undefined;
	_ackDirty = false;
	_ackDue = false;
	...
	constructor(
		options: CreateRpcSessionDeliveryOptions,
		createDuplicateEvidence: RpcSessionDuplicateEvidenceFactory,
	) {
		this._codec = options.codec;
		this._duplicates = createDuplicateEvidence({
			reserveRetainedBytes: options.reserveDuplicateBytes,
		});
	...
	receiveEnvelope(envelope: RpcMessageEnvelope): boolean {
		...
		if (envelope.seq <= this._receivedThrough) {
			if (!this._duplicates.matches(envelope)) {
				this._fault(
					RpcCloseReasonEnum.protocolFault,
					new Error(
						"RPC duplicate sequence rewrites retained semantic evidence.",
					),
				);
				return false;
			}
		...
		this._duplicates.retain(envelope);
	...
	queueStream(prepare: () => IRpcReplayReservation | undefined): () => void {
		this._pendingStreams.push(prepare);
		this.pump();
		return () => {
			const index = this._pendingStreams.indexOf(prepare);
			if (index !== -1) this._pendingStreams.splice(index, 1);
		};
	}
	...
	pump(): void {
		...
		const hasPending =
			this._invocations.hasPending || this._pendingStreams.length !== 0;
		...
		if (shouldSendControl) {
			const selected = takeRpcStreamControl(
				this._controlQueue,
				this._lastControlLane,
			);
			if (selected === undefined) return;
			this._lastControlLane = getRpcStreamLane(selected.message);
			this._preferControl = false;
			this._probeSentLast = false;
			this._admitSemantic(binding, selected);
			return;
		}
		if (hasPending) {
			this._preferControl = true;
			this._probeSentLast = false;
			this._admitPending(binding);
			return;
		}
	...
	_admitPending(binding: IRpcSessionConnection): void {
		const stream = this._pendingStreams[0];
		if (
			stream !== undefined &&
			(this._preferStream || !this._invocations.hasPending)
		) {
			this._preferStream = false;
			this._pendingStreams.shift();
			const replay = stream();
			if (replay !== undefined) this._admitSemantic(binding, replay);
			else queueMicrotask(() => this.pump());
			return;
		}
		this._preferStream = true;
