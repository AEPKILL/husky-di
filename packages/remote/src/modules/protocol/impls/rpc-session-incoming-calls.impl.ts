...
export class RpcSessionIncomingCallsImpl implements IRpcSessionIncomingCalls {
	...
	readonly _normalizeApplicationValue: CreateRpcSessionIncomingCallsOptions["normalizeApplicationValue"];
	readonly _isDraining: () => boolean;
	readonly _getIncomingStreamCount: () => number;
	...
	constructor(options: CreateRpcSessionIncomingCallsOptions) {
		...
		this._getIncomingStreamCount = options.getIncomingStreamCount ?? (() => 0);
	...
	receiveCall(message: RpcCallMessage): void {
		...
		const metadata =
			message.metadata === undefined
				? undefined
				: (this._normalizeApplicationValue(
						message.metadata,
		...
		if (
			this._isDraining() ||
			this._retention.incomingCount + this._getIncomingStreamCount() >=
