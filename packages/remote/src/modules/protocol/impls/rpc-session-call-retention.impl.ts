 ...
 * @author AEPKILL
 * @created 2026-09-09 23:33:27 15:00:00
 */
...
export class RpcSessionCallRetentionImpl implements IRpcSessionCallRetention {
	...
	reserveReplay(
		...
		return this._reserveReplay(message, undefined, onCommit, onRelease);
	}
	...
	_reserveReplay(
		...
		try {
			...
			if (
				(message.kind === RpcWireRecordKindEnum.call ||
					message.kind === RpcWireRecordKindEnum.streamOpen) &&
				message.metadata !== undefined
			) {
				// Separately bounded arguments and metadata share one wire-tree budget.
				this._codec.decode(maximumEnvelope, RpcDecodePhaseEnum.active);
			}
		} catch {
			return undefined;
		}
