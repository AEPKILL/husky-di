...
export class RpcProtocolAcceptorImpl implements IRpcProtocolAcceptor {
	...
	async _acceptFresh(
		...
		const profile = request.profiles.includes(RPC_PROFILE)
			? RPC_PROFILE
			: RPC_UNARY_PROFILE;
		if (!request.profiles.includes(profile)) {
			await this._rejectFresh(attempt, "unsupported-profile");
			return;
		}
		const prepared = this._prepareFresh(attempt, profile);
		...
		const accept = Object.freeze({
			...
			profile: prepared.session.profile ?? RPC_PROFILE,
	...
	async _acceptResume(
		...
		if (
			session === undefined ||
			request.profile !== (session.profile ?? RPC_PROFILE)
		) {
		...
		const accept = Object.freeze({
			kind: RpcWireRecordKindEnum.accept,
			profile: session.profile ?? RPC_PROFILE,
