...
export class RpcSessionInvocationsImpl implements IRpcSessionInvocations {
	...
	readonly _getPendingStreamBytes: () => number;
	...
	constructor(
		...
		this._getPendingStreamBytes = options.getPendingStreamBytes ?? (() => 0);
	...
	get pendingBytes(): number {
		return this._pendingInvocationBytes;
	}
	...
	prepareInvocation(
		...
		const cannotReserveInvocation =
			...
			pendingCharge >
				this._maximumPendingBytes -
					this._pendingInvocationBytes -
					this._getPendingStreamBytes();
