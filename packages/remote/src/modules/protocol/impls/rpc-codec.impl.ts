/**
 * @overview Bounded RFC 8259 Codec and profile-aware record validation for RPC/1.
 * @author AEPKILL
...
import type {
	IRpcCodec,
	RpcDecodedRecord,
} from "@/modules/protocol/interfaces/rpc-codec.interface";
import type {
	RpcJsonRecord,
	RpcJsonValue,
...
export class RpcCodecImpl implements IRpcCodec {
	readonly _profile: string;

	constructor(profile: string = RPC_PROFILE) {
	...
	public decode<TPhase extends RpcDecodePhaseEnum>(
		...
		switch (phase) {
			...
			case RpcDecodePhaseEnum.freshAccept:
				return validateRpcRecord(
					record,
					rpcFreshAcceptSchema as unknown as ZodType<RpcJsonRecord>,
					"RPC fresh attempt did not receive accept.",
				) as RpcDecodedRecord<TPhase>;
			case RpcDecodePhaseEnum.resumeOutcome:
				return validateRpcRecord(
					record,
					rpcResumeOutcomeSchema as unknown as ZodType<RpcJsonRecord>,
					"RPC resume attempt did not receive accept or reject.",
				) as RpcDecodedRecord<TPhase>;
			case RpcDecodePhaseEnum.active:
				if (
					this._profile === RPC_UNARY_PROFILE &&
					record.kind === "message" &&
					typeof record.message === "object" &&
					record.message !== null &&
					"kind" in record.message &&
					String(record.message.kind).startsWith("stream-")
				) {
					throw new Error("RPC/1 does not support stream records.");
				}
				return validateRpcRecord(
					record,
					rpcActiveRecordSchema as unknown as ZodType<RpcJsonRecord>,
