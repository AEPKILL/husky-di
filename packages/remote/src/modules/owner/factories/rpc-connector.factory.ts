...
import type { RpcInterceptor } from "@/modules/peer";
import type {
...
export function createRpcConnector(
	...
	return new RpcConnectorImpl({
		...
		createSessionOwnership: (ownershipOptions) =>
			createRpcConnectorSessionOwnership({
				...ownershipOptions,
				peerEnvironment: {
					...ownershipOptions.peerEnvironment,
					interceptor: parsed.interceptor,
				},
			}),
...
function parseConnectorOptions(options: RpcConnectorOptions | undefined): {
	...
	const { protocolFactory, runtimePolicy, interceptor } = optionsResult.data;
