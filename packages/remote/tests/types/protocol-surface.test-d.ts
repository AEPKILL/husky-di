...
test("RPC-INTERCEPT-005 RPC-SPI-003 exposes optional branded metadata record snapshots", () => {
	expectTypeOf<IRpcProtocolCallRequest["metadata"]>().toEqualTypeOf<
		IRpcApplicationSnapshot<IRpcApplicationRecord> | undefined
	>();
	// @ts-expect-error Protocol receives a Framework snapshot, not unbranded metadata.
	const metadata: NonNullable<IRpcProtocolCallRequest["metadata"]> = {
		value: { traceId: "trace-1" },
		weight: 21,
	};
	void metadata;
});
...
test("RPC-TRANSPORT-012 exposes no structural transport security claim", () => {
	expectTypeOf<
		keyof import("../../src/transport").IRpcConnection
	>().toEqualTypeOf<"message$" | "send" | "close">();
	expectTypeOf<
		keyof import("../../src/transport").IRpcConnectorAdapter
	>().toEqualTypeOf<"connection$" | "connect">();
	expectTypeOf<
		keyof import("../../src/transport").IRpcAcceptorAdapter
	>().toEqualTypeOf<"connection$" | "listen">();
});
