...
describe("exposure registries and remote facades", () => {
	it("RPC-STREAM-001 validates static data members atomically without invoking accessors or subscribing", () => {
		const identifier = createServiceIdentifier<{
			watch(): Observable<number>;
			updates$: Observable<number>;
		}>("IExposureStreamService");
		const descriptor = createRemoteServiceDescriptor(identifier, {
			wireName: "example.exposure-stream.v2",
			members: {
		...
		const invalid = Object.defineProperty({ watch: () => of(1) }, "updates$", {
			get() {
				accesses += 1;
				return source;
			},
		});
		expect(() => connector.peer.expose(descriptor, invalid as never)).toThrow(
			TypeError,
		);
	...
	it("RPC-CALL-002 validates the dedicated cancellation slot before peer availability", async () => {
		...
		const descriptor = createRemoteServiceDescriptor(ICalculatorService, {
			wireName: "example.calculator.v1",
			members: { cancel: { kind: "function", cancelable: true } },
		});
		const remote = connector.peer.resolve(descriptor);
