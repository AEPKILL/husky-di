...
export const interceptedCalculatorDescriptor = createRemoteServiceDescriptor(
	ICalculatorService,
	{
		wireName: "example.intercepted-calculator.v1",
		members: {
			add: { kind: "function" },
...
export async function createInterceptedPair(
	...
	const cancelHandler = vi.fn(
		async (value: string, _signal: AbortSignal) => value,
	);
	const implementation = { add: handler, cancel: cancelHandler };
	connector.peer.expose(interceptedCalculatorDescriptor, implementation);
	acceptor.expose(interceptedCalculatorDescriptor, implementation);
	...
	const remote = connector.peer.resolve(interceptedCalculatorDescriptor);
	const calls = () =>
		network.records
			.filter((record) => record.value.kind === "message")
			.map(
				(record) => record.value.message as Readonly<Record<string, unknown>>,
			)
