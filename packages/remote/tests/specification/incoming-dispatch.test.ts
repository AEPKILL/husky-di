...
describe("custom Protocol incoming calls", () => {
	...
	it("RPC-INTERCEPT-005 RPC-SPI-006 RPC-RESOURCE-001 charges incoming metadata against the args subcap and releases it at terminal settlement", async () => {
		const session: IRpcProtocolSession = {
			prepareInvocation: () => undefined,
			forceClose() {},
		};
		...
		const args = host.normalizeApplicationArguments([]);
		const metadata = host.normalizeApplicationValue({
			traceId: "a".repeat(300_000),
			baggage: "b".repeat(300_000),
		}) as IRpcApplicationSnapshot<IRpcApplicationRecord>;
		expect(args.weight).toBe(2);
		expect(metadata.weight).toBeGreaterThan(600_000);
		const request = {
			service: "unknown.service",
			method: "unknownMethod",
			args,
		...
		expect(
			events.filter((event) => event.type === "call-started"),
		).toHaveLength(1);
		first.call.finish({
			type: RpcCallTerminalTypeEnum.failed,
			code: first.code,
		...
		const afterRelease = consumeIncomingCall(sessionHost, request);
		if (afterRelease?.kind !== "unknown")
			throw new Error("Expected metadata capacity to be released.");
		afterRelease.call.finish({
			type: RpcCallTerminalTypeEnum.failed,
			code: afterRelease.code,
		});
