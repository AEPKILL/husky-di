...
test("RPC-STREAM-002 RPC-STREAM-004 RPC-STREAM-005 RPC-STREAM-006 RPC-STREAM-010 RPC-RELEASE-002 runs method/static streams, cancel, Recovery, and v1 fallback", async ({
	page,
}) => {
	await page.goto(browserOrigin);
	await page.addScriptTag({ path: bundlePath });
	const result = await page.evaluate(async () => {
		const fixture = (
			globalThis as typeof globalThis & {
				readonly HuskyDiBrowserEvidence: {
					runRpcBrowserStreams(): Promise<unknown>;
				};
			}
		).HuskyDiBrowserEvidence;
		return fixture.runRpcBrowserStreams();
	});
	expect(result).toEqual({
		methodValues: [1, 2, 3],
		sourceError: "handler-failed",
		staticLazy: true,
		staticStarts: 1,
		staticStops: 1,
		staticCompletions: 1,
		firstValues: [7],
		secondValues: [7],
		recoveredValues: [10, 11, 12],
		methodStarts: 1,
		methodStops: 1,
		cancellationNotifications: [],
		stableStaticFacade: true,
		v1: {
			unaryResult: 42,
