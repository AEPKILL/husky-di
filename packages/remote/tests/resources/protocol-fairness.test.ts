...
describe("stream lane fairness", () => {
	it("RPC-STREAM-007 RPC-SCHEDULE-001 alternates busy stream lanes while preserving each stream order", async () => {
		const harness = createRpcDirectSessionHarness();
		const gate = Promise.withResolvers<void>();
		harness.setSendSettlement(gate.promise);
		for (const [streamId, value] of [
			["1", 1],
			["1", 2],
			["1", 3],
			["2", 10],
			...
			harness.session._delivery.queueSemantic({
		...
		expect(
			harness.sent.map((record) => (record.message as { value: number }).value),
		).toEqual([1, 10, 2, 11, 3]);
