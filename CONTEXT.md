...
- **Remote Stream Admission**: The receiver-side linearization point at which an ordinarily accepted stream request becomes retained Logical Session state and application source acquisition becomes permitted. Resource and semantic rejection are alternate retained dispositions rather than Remote Stream Admission and must not execute the stream method, read its property source, or subscribe to it.
- **Stream Source Acquisition**: The Source Side's one-time evaluation of an admitted Application Stream Member to obtain its application Observable source. The Framework transiently holds the acquired Observable, while the application continues to own its hot, cold, sharing, and replay semantics.
- **Source Subscription Admission**: The receiver-side linearization point at which the Framework invokes the acquired application Observable's `subscribe()` exactly once and assumes responsibility for its teardown. It remains distinct from Remote Stream Admission because source acquisition can fail before a Source Subscription begins.
...
- **Observer Delivery**: The Subscriber Side's invocation of its Observer's `next()` with one Stream Item. It does not by itself acknowledge or retire Protocol evidence for that item.
...
- **Stream Terminal**: The single authoritative semantic outcome of a Logical Stream Subscription, selected by source completion or failure, caller cancellation, Session termination, or forced convergence. Its Subscriber-Side and Source-Side projections need not become known simultaneously, and it remains distinct from an observer complete/error notification, Source Subscription teardown, and later retirement of retained Protocol evidence.
...
- **Source Teardown**: The remote Framework's one-time release of its Source Subscription in response to terminal, cancellation, or force. It can finish after local observation has ended and cannot replace an already selected Stream Terminal.
