...
- `@husky-di/remote`: Descriptor-driven bidirectional unary RPC over application-supplied Transport Adapters.
...
- **Remote Service Descriptor**: A description relating one local `ServiceIdentifier` to an explicit Wire Service Name and a non-empty unary method allowlist. Its readonly metadata consists of `serviceIdentifier`, `wireName`, and `methods`.
...
- **Remote Facade**: A locally resolved object containing the Descriptor's selected asynchronous methods. The reserved property `then` prevents it from becoming a JavaScript thenable.
