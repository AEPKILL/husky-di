...
- **Application Stream Member**: A remotely exposed method returning an RxJS Observable or an explicitly allowlisted read-only Observable property. Both forms enter the same Application Stream lifecycle when subscribed; they differ only in how the remote source is acquired.
...
- **Remote Service Descriptor**: An opaque description of a remotely accessible service that relates one local `ServiceIdentifier` to an explicit Wire Service Name and a non-empty allowlist of unary or Application Stream Members. The property name `then` is reserved so a remote service facade cannot become a JavaScript thenable.
