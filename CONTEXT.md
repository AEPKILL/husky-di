...
- **Handler Settlement**: The first authoritative fulfillment, rejection, or synchronous throw produced by an already-dispatched handler computation. Selecting a call terminal or aborting its signal is not Handler Settlement.
...
- **Invocation Preparation**: The atomic Protocol transition that acquires capacity and creates a Pending Invocation while withholding Call Identity, ledger admission, and send eligibility until `start()`.
- **Incoming Reservation Scope**: The single synchronous Framework callback in which a Protocol records its durable incoming disposition and commits the offered handler or semantic-rejection capability exactly once. The commit capability cannot escape the scope.
...
- **Remote Service Descriptor**: A description of a remotely callable service with readonly `serviceIdentifier`, `wireName`, and `methods` metadata, relating one local `ServiceIdentifier` to an explicit Wire Service Name and a non-empty unary method allowlist. The property name `then` is reserved so a remote service facade cannot become a JavaScript thenable.
- **Wire Identifier**: The common wire-level name grammar used for peer-visible RPC identities such as Wire Service Names and RPC method names. It constrains stable wire identities without tying them to local `ServiceIdentifier` equality or metadata.
...
- `docs/adr/0001-asymmetric-rpc-call-seam.md` records the Accepted Remote RPC call-seam decision; consult it with the Remote specification when changing Protocol call admission or phase ownership.
