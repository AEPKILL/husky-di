...
- **RPC Protocol**: The replaceable RPC semantics module used by a Topology Owner to carry Logical Sessions, calls, and Application Streams over Physical Connections. Every conforming Protocol preserves the same public v1 guarantees while choosing its own wire contract and internal organization.
...
- **Message Receipt ACK**: The single cumulative per-direction confirmation used for every sequenced Protocol message: all messages through the cursor have been validated and given a Session-retained, replay-suppressing disposition. Cursor zero means none has been admitted; the ACK has no message-kind subtype and does not prove handler or source execution, cancellation or terminal victory, Observer callback return, application processing, external effects, or complete evidence retirement.
...
- **Definite Non-Execution**: An evidence-backed conclusion that an RPC application operation did not reach its operation-specific remote admission, so its handler or stream source acquisition cannot have executed. It allows a new attempt without risking duplicate remote execution of that operation.
- **Outcome Unknown**: A terminal caller-visible condition in which an outgoing call or Logical Stream Subscription crossed its Outgoing Admission but no authoritative terminal disposition remains provable, so remote execution, delivered items, or application effects may have occurred. The RPC framework does not automatically retry it as a new operation.
- **RpcException**: A Framework-created caller-visible RPC failure whose closed code is its only stable branch field. Remote failures never expose the remote thrown value, message, stack, or cause; a trusted same-side infrastructure failure may remain a local cause where its operation contract permits one.
...
- **Application Work Slot**: A direction-local capacity unit shared by unary invocations and Application Streams against both Logical Session and Topology Owner allowances. Every Pending or admitted operation owns one; an Application Stream additionally owns an Active Stream Slot.
- **Active Stream Slot**: A direction-local, stream-only subset of Application Work capacity. Every Pending or admitted Logical Stream Subscription owns one alongside its Application Work Slot, so this allowance can constrain stream admission but cannot expand total Application Work capacity.
- **Application Stream Member**: A remotely exposed method returning a direct RxJS Observable from ordinary Application Value arguments, or an explicitly allowlisted required, read-only `$` Observable property. Both forms enter the same Application Stream lifecycle when subscribed; they differ only in how the remote source is acquired, and stream cancellation belongs to the Caller Stream Subscription rather than an `AbortSignal` parameter.
- **Application Stream**: The remotely carried sequence and lifecycle created by one Caller Stream Subscription to an Application Stream Member. It is resource-owning work, unlike an Observation Stream.
- **Subscriber Side**: The side of one Application Stream that owns its Caller Stream Subscription and Observer. It originates the Subscription Direction regardless of whether its Topology Owner is a Connector or Acceptor.
- **Source Side**: The side of one Application Stream that exposes its Application Stream Member and owns source acquisition, the Source Subscription, and Source Teardown. It is independent of fixed client/server or Connector/Acceptor roles.
- **Subscription Direction**: The direction from the Subscriber Side to the Source Side in which stream start and cancellation intent originate. Stream items and source terminals normally travel in the reverse direction.
- **Remote Observable**: A cold caller-side Observable facade for one Application Stream Member. Creating, accessing, or retaining it performs no remote work; each subscription creates one independent Caller Stream Subscription.
- **Caller Stream Subscription**: One caller-side subscription to a Remote Observable and the lifecycle root of one independent remote stream. Unsubscribe immediately ends local observation and requests remote teardown without implying that remote work has already stopped.
- **Application Stream Observation**: One side-local, payload-free started/finished telemetry pair for a qualifying Application Stream attempt. It reports that side's lifecycle outcome and is not a Stream Identity, a global Stream Terminal, or proof that remote work, Source Teardown, or evidence retirement has finished.
- **Local Stream Subscription Admission**: The Subscriber Side's linearization point after subscription preflight and finite local capacity acquisition that creates a Pending Stream Subscription. Failure before it creates no remote work or retained stream state.
- **Pending Stream Subscription**: A locally admitted Caller Stream Subscription that has no Stream Identity and has not reached Outgoing Stream Admission. Retraction before that admission ends only the pending and local-observation lifetimes, proves that no remote stream work can execute, and creates no Stream Terminal.
- **Outgoing Stream Admission**: The atomic transition that assigns a Pending Stream Subscription its Session-scoped Stream Identity, commits its immutable retained start evidence, and first invokes the current Physical Connection's `send()`. Before this point the work remains retractable and identity-free; afterward cancellation is cooperative because remote execution can no longer be disproved.
- **Logical Stream Subscription**: One bidirectional Application Stream subscription identified within a Session Incarnation and Subscription Direction. It is distinct from a unary Logical Call and preserves its identity, admission state, and established Source Subscription across successful Physical Connection Recovery.
- **Stream Identity**: The identity of a Logical Stream Subscription within one Session Incarnation and Subscription Direction, created at Outgoing Stream Admission and never reused during that incarnation. Its opaque `streamId` is Protocol-owned and remains independent of member routing metadata, a Physical Connection, a caller RxJS object, and telemetry correlation.
- **Stream Ordinal**: The Default Protocol's direction-local, contiguous Logical Stream Subscription sequence, carried by the opaque `streamId` and allocated at Outgoing Stream Admission. Its allocation and retained-disposition high-watermarks survive individual stream retirement until Session terminal so a Stream Identity cannot be reused without permanent per-stream tombstones.
- **Remote Stream Route Capture**: The receiver-side linearization point at which a capacity-reserved, exact-matched incoming stream request freezes its immutable exposure entry before Remote Stream Admission. Exposure cleanup affects only requests that have not reached this point; capture alone does not permit source acquisition, but cleanup cannot revoke it while Protocol disposition is pending.
- **Remote Stream Admission**: The receiver-side linearization point at which an ordinarily accepted stream request becomes retained Logical Session state, commits its previously reserved finite Source Start Job capacity and minimum protected terminal/control capacity using the captured member route, and permits source acquisition.
- **Remote Stream Resource Rejection**: The alternate retained disposition for a valid expected stream request when ordinary source-start capacity is unavailable but protected rejection capacity remains. It commits protected `unavailable`, guarantees no subscription-specific source acquisition, and is not Remote Stream Admission; inability to retain even that protected disposition is a Session resource fault.
- **Remote Stream Semantic Rejection**: The alternate retained disposition for an unknown service, unknown member, or member-kind mismatch. It commits safe `unknown-service` or `unknown-member`, guarantees no subscription-specific source acquisition, and is not Remote Stream Admission.
- **Source Start Job**: The Source Side's single scheduled application work created by Remote Stream Admission to acquire the source and begin its exactly-once subscription attempt. It ends after that attempt; an established Source Subscription has an independent lifetime.
- **Stream Source Acquisition**: The Source Side's one-time materialization of an admitted stream's application Observable: it adopts an exposure-time-captured data-property source or invokes the captured stream method or getter with its receiver. The Framework transiently holds the result, while the application continues to own its hot, cold, sharing, and replay semantics.
- **Source Subscription Admission**: The receiver-side linearization point at which the Framework begins the exactly-once call to the acquired source's `subscribe()` and assumes responsibility for any Source Subscription and teardown that materializes. It precedes `subscribe()` return, so synchronous notifications, terminal, or throw occur after admission and never trigger a retry.
- **Source Subscription**: The remote-side RxJS subscription through which the Framework consumes one application Observable source for a Logical Stream Subscription. Physical Connection replacement does not recreate it; source semantics and underlying producer sharing remain application-owned.
- **Source Emission**: A candidate value passed by an application Observable source to the Framework through `next()`. It is not yet a Stream Item and remains outside retained Protocol state until Stream Item Admission succeeds.
- **Stream Item Admission**: The linearization point at which a valid Source Emission has been normalized to a detached immutable Application Value snapshot, acquired its finite capacity, and become retained Protocol work. Failure before this point creates no Stream Item.
- **Stream Receive Slot**: The Subscriber Side's retained capacity for one maximum-legal Stream Item. Each Logical Stream Subscription owns exactly one; it is a Framework capacity guarantee, not an RxJS demand signal or a burst buffer.
- **Stream Admission Credit**: Finite item-count permission backed one-for-one by a Stream Receive Slot that a Source Emission must consume to cross Stream Item Admission. It bounds Framework and Protocol work rather than demand on the application producer.
- **Stream Credit Horizon**: The cumulative monotonic count of Stream Item Admissions the Subscriber Side has granted for one Logical Stream Subscription. The Subscriber may retain one coalesced newer due horizon, while the Source Side can spend only the highest horizon it has durably accepted, so delayed transmission, duplicate evidence, or replay cannot mint credit twice.
- **Stream Item Ordinal**: The contiguous, one-based position assigned to a Stream Item at Stream Item Admission within one Stream Identity. Replay preserves it; it is distinct from Protocol message sequence, Stream Credit Horizon, and Application Value equality.
- **Stream Item**: The normalized Application Value snapshot created by Stream Item Admission for one Logical Stream Subscription. It remains distinct from its source value, wire representation, Observer Delivery, acknowledgement, and retained evidence.
- **Stream Item Disposition**: The Subscriber Side's Session-retained, replay-suppressing decision that one identified Stream Item will be projected to the still-open Observer at most once or suppressed because local observation has closed. It is committed before any Observer callback and is not evidence of callback return, application processing, durability, or an external effect.
- **Observer Delivery**: The Subscriber Side's invocation of its Observer's `next()` with one Stream Item. Its synchronous return only releases that delivery's local occupancy; it does not prove application processing, durability, acknowledgement, or evidence retirement.
- **Source Terminal**: A completion or error notification emitted by an application Observable source. It competes to select the Stream Terminal but remains distinct from caller cancellation and forced convergence.
- **Stream Cancellation**: Caller unsubscribe immediately and idempotently ends local observation; before Outgoing Stream Admission it retracts identity-free Pending work, and afterward it creates a cooperative cancellation intent. Only Source-Side admission of that intent competes for the Stream Terminal, so unsubscribe alone never proves that remote work stopped.
- **Stream Terminal**: The single authoritative semantic outcome of a Logical Stream Subscription, selected by remote rejection, source completion or safe `handler-failed`, Source-Side cancellation admission, Stream Overflow, Session termination, or forced convergence. Its two side projections need not become known simultaneously, and it remains distinct from local unsubscribe, an Observer Terminal Notification, Source Teardown, and later Stream Evidence Retirement.
- **Stream Terminal Boundary**: The final Stream Item Ordinal retained with a Stream Terminal, or zero when no item was admitted. It proves the exact admitted-item prefix that must receive Subscriber-Side dispositions before the terminal can be projected, without making the terminal consume Stream Admission Credit or share the item ordinal sequence.
- **Stream Overflow**: The Stream Terminal selected when a valid Source Emission cannot obtain Stream Admission Credit or ordinary retained capacity after Remote Stream Admission. The emission creates no Stream Item, and the explicit terminal follows every earlier admitted item.
- **Observer Terminal Notification**: The completion or safe error actually delivered to the caller's Observer. Caller unsubscribe closes local observation without manufacturing this notification, and Protocol replay cannot deliver it more than once.
- **Source Teardown**: The Source-Side Framework's one-time release attempt for its Source Subscription, begun after the Stream Terminal wins and independently of Protocol acknowledgement or evidence retirement; it does not prove that every application-owned producer stopped. A teardown failure is a local secondary cleanup failure that cannot replace the Stream Terminal, expose its raw error remotely, or escalate a healthy Session or Topology Owner.
- **Source Teardown Incident**: The payload-free local fact that a Source Teardown attempt failed. It is orthogonal to the Stream Terminal and appears publicly only as `sourceTeardownFailed: true` on the Source Side's matching finished Application Stream Observation.
- **Stream Evidence Retirement**: The Protocol's release of retained Stream Identity, delivery, cancellation, and terminal evidence after it is no longer needed for Recovery or deduplication. It is later than logical terminal selection and independent of Source Teardown.
- **Stream Evidence Replay**: Retransmission of the same retained stream start, item, credit, cancellation, or terminal evidence with unchanged Protocol message identity and semantic content after receipt is uncertain. It creates no new Stream Identity, source acquisition, Stream Item Admission, credit, terminal, or Observer notification.
- **Observation Stream**: A public state, membership, telemetry, or Transport Observable whose subscriptions only observe an independently owned resource. Unlike a Remote Observable, changing its subscriber count never creates, starts, stops, or owns the observed resource.
...
- **Session Proof Key**: A private symmetric key derived from the one-time Session secret delivered inside a Protected Transport. It authenticates resume role, profile, cursor, attempt and accept/reject transcript; it proves continuity authority, not a business-user identity.
- **Resume Attempt**: The initiator-owned, strictly increasing and never-reused safe integer included in each authenticated resume request. It can have gaps and lets a higher attempt recover after a lost accept without replaying an older proof.
...
- **Connection Fencing**: The Session rule that prevents a replaced Physical Connection and its late effects from changing retained Session, call, or Application Stream state after a newer Binding Epoch is installed.
...
- **Graceful RPC Shutdown**: The `shutdown()` Topology Owner path: atomically stop new application/bootstrap admission and drain the finite pre-cutoff unary work and active Application Streams belonging to eligible connected Sessions before closing each drained binding. Recovering Sessions are forced immediately, and the absolute grace interval escalates unfinished work to Forced RPC Close; graceful shutdown is not a Physical Connection capability.
- **Forced RPC Close**: The `close()` Topology Owner path, or the fallback reached when graceful shutdown expires or loses a binding. It immediately applies Session-terminal outcomes to calls and Logical Stream Subscriptions, performs Source Teardown, aborts cancelable handlers, drops unsent Protocol intent, and Direct Closes Connections without sending Session-close.
- **Shutdown Drain Cutoff**: The no-await linearization point of the first `shutdown()` that changes the Owner and its currently connected Peers to draining, rejects later local work, and freezes the finite set of earlier unary and Application Stream work allowed to finish. Peers already recovering at that point take the forced path instead.
...
- **Session Recovery**: Reattaching a Logical Session to a replacement Physical Connection while preserving its peer identity and retained call and Application Stream state. Pending Stream Subscriptions remain identity-free, admitted subscriptions retain their Stream Identity, and established Source Subscriptions are not recreated; a source terminal during Recovery immediately competes for the Stream Terminal and, if it wins, tears down while its retained evidence waits for replay.
- **Recovery Replay Barrier**: The per-sending-direction replacement-binding ordering constraint that first retransmits the finite frozen set of previously sequenced, unconfirmed Protocol messages in original sequence order. Newly retained work may queue behind the barrier but cannot extend it or receive a new message sequence until it clears.
- **Ordinary Outbound Evidence**: The retained obligation for an ordinary outbound semantic record from capacity-backed admission before sequence allocation through Message Receipt ACK or equivalent authenticated cursor coverage. Its reservation transfers between retained-unsent and replay state without double charging and excludes protected terminal and cancellation evidence.
- **Protected Convergence Obligation**: Non-borrowable Session capacity reserved so already-existing application work can still express one safe Source-Side terminal or one caller-originated cancellation after ordinary capacity is exhausted. It is established before the work can need convergence and cannot be manufactured opportunistically at terminal time.
...
- **RPC Runtime Policy**: The immutable owner-construction rules that bound Session/owner bytes, shared Application Work and its Active Stream subset, application callbacks, and Protocol deadlines. It has finite validated defaults, no runtime setters or per-peer overrides, and is supplied to every conforming Protocol runtime.
- **Retained-Byte Ledgers**: The two-level atomic accounting boundary formed by each Session's aggregate child ledger and its Topology Owner ledger. Protected Session reserves, Stream Receive Slots, Session-attributable Endpoint ingress, Pending/ordinary-evidence entries, and incoming application-work payloads acquire the applicable ledgers together; one frozen idempotent reservation follows each retained representation until its evidence-backed release point.
- **Payload-Free RPC Telemetry**: The common owner event surface contains peer/correlation identity, direction, service/member metadata, phase, safe outcome code, and bounded timing/counts, but never application payloads, raw errors, wire/session/call/stream identities, proofs, nonces, or secrets. Payload diagnostics remain application-owned instrumentation at caller/handler boundaries.
- **Remote Service Descriptor**: An opaque description that relates one local `ServiceIdentifier` to an explicit Wire Service Name and one non-empty member namespace classifying selected entries as unary, stream method, or stream property. The member name `then` is reserved so a remote service facade cannot become a JavaScript thenable.
