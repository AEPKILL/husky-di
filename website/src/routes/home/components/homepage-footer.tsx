...
export function HomepageFooter() {
  return (
    <footer className="border-t border-border bg-surface-deep">
      <div className="mx-auto flex w-full max-w-300 flex-col items-center justify-between gap-4 px-6 py-6 md:flex-row">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6">
          {HOME_PAGE_FOOTER_NAV_ITEMS.map((item) =>
            "href" in item && item.href ? (
              <a
                className="font-mono text-xs uppercase tracking-widest text-page-muted transition-colors hover:text-page-fg"
                href={item.href}
                key={item.label}
                rel="noreferrer"
                target="_blank"
              ...
              <span
                className="font-mono text-xs uppercase tracking-widest text-page-muted"
                key={item.label}
              >
                {item.label}
              </span>
